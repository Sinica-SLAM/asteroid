...
class _InstrumentBackboneEnc(nn.Module):
    ...
    def __init__(
        ...
        self.hidden_size = hidden_size
        self.film = Film(mu_vec_size, self.max_bin * nb_channels)
    ...
    def forward(self, x, mu_vec, shapes):
        ...
        mu_vec_mean = torch.mean(mu_vec,dim=0).squeeze()
        x = self.film(x.reshape(-1, nb_channels * self.max_bin),mu_vec_mean)
        x = self.enc(x)
...
class _InstrumentBackboneDec(nn.Module):
    ...
    def forward(self, x, mu_vec, shapes):
        ...
        mu_vec_mean = torch.mean(mu_vec,dim=0).squeeze()
        x = self.film(x.reshape(-1, x.shape[-1]),mu_vec_mean)
        x = self.dec(x)
